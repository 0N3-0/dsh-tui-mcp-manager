...
export function normalizeSetRecord(input: unknown): ManagedSetRecord {
  ...
  return {
    ...
    serverIds: [...new Set(value.serverIds as string[])],
  }
}

/** Apply the union of all active Sets in one complete patch update. */
export function applyActiveSetsToServers(
  ...
  if (unknown.length > 0) throw new Error(`set references unknown server(s): ${unknown.join(', ')}`)
  const setIds = new Set(sets.map((set) => set.id))
  const unknownActive = activeSetIds.filter((id) => !setIds.has(id))
  if (unknownActive.length > 0) throw new Error(`active set does not exist: ${unknownActive.join(', ')}`)
...
function parseDocument(content: string, filename: string): SetDocument {
  ...
  if (parsed === undefined || parsed === null) return { version: 1, sets: [] }
  const value = asObject(parsed, filename)
  if (value.version !== 1) throw new Error(`${filename} version must be 1`)
  if (!Array.isArray(value.sets)) throw new Error(`${filename} sets must be an array`)
  const sets = value.sets.map(normalizeSetRecord)
  const ids = new Set<string>()
  for (const set of sets) {
    if (ids.has(set.id)) throw new Error(`${filename} contains duplicate set id ${JSON.stringify(set.id)}`)
    ids.add(set.id)
  }
  ...
  return {
    version: 1,
    sets,
    ...(value.initialized === undefined ? {} : { initialized: value.initialized as boolean }),
    ...(value.activeSets === undefined ? {} : { activeSets: [...new Set(value.activeSets as string[])] }),
  }
}
...
export class ProfileSetStore {
  ...
  async read(): Promise<SetStoreSnapshot> {
  ...
  async write(
    ...
    if (activeSetIds.some((id) => !known.has(id))) throw new Error('active sets must reference existing sets')
