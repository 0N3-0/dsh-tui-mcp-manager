...
export function serverStateGlyph(state: McpServerView['state']): string {
  switch (state) {
...
export function serverStateColor(state: McpServerView['state']): ServerStateColor {
  switch (state) {
...
export function renderServerDetailView({
  React,
  ui,
  lang,
  server,
  ...
  const row = (label: string, value: string, color?: ServerStateColor) => h(
    Box,
  ...
  if (tab === 'overview') {
    ...
    return h(
      React.Fragment,
      null,
      row(text(lang, 'runtime'), runtimeStateText(lang, server.state), serverStateColor(server.state)),
  ...
  if (tab === 'tools') {
    ...
    return h(
      ...
      h(Box, { height: 1 }),
      h(Text, { bold: true, color: 'suggestion' }, `${selectedToolIndex + 1}/${server.tools.length}  ${tool.name}`),
