...
function stateGlyph(state: McpServerView['state']): string {
  switch (state) {
...
function stateColor(state: McpServerView['state']): 'success' | 'warning' | 'error' | 'inactive' {
  switch (state) {
...
function yesNo(lang: SceneLanguage, value: boolean): string {
...
export function createMcpManagerScene(
  ...
  return function McpManagerScene(props: TuiSceneProps) {
    ...
    const row = (label: string, value: string, color?: 'success' | 'warning' | 'error' | 'inactive') => h(
      Box,
      ...
      ...shownNav.map((item) => {
        ...
        if (item.kind === 'server') {
          return h(
            ...
            h(Text, { color: focused ? undefined : stateColor(item.server.state) }, focused ? '\u276f ' : '  '),
            h(Text, { color: focused ? undefined : stateColor(item.server.state) }, `${stateGlyph(item.server.state)} `),
    ...
    const renderOverview = (server: McpServerView) => h(
      React.Fragment,
      null,
      row(text(lang, 'runtime'), runtimeStateText(lang, server.state), stateColor(server.state)),
      row(text(lang, 'transport'), server.transport),
      row(text(lang, 'namespace'), server.serverName),
      row(text(lang, 'endpoint'), server.transport === 'stdio'
      ...
      server.error !== undefined && h(
        ...
        h(Text, { color: 'error', wrap: 'wrap' }, server.error),
      ),
      h(Box, { marginTop: 1 }, h(Text, { color: 'subtle', wrap: 'wrap' }, text(lang, 'previewLimit'))),
    )

    const renderTools = (server: McpServerView) => {
      if (server.tools.length === 0) return h(Text, { color: 'subtle' }, text(lang, 'noTools'))
      ...
      if (!toolDetailOpen) {
        ...
        const start = Math.min(
          Math.max(0, selectedToolIndex - Math.floor(capacity / 2)),
          Math.max(0, server.tools.length - capacity),
        )
        ...
        return h(
          ...
          ...visibleTools.map((item, offset) => {
            ...
            return h(
              ...
              h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '),
              h(Text, { bold: selectedItem, wrap: 'truncate-end' }, item.name),
              h(Box, { flexGrow: 1 }),
              h(Text, { color: focused ? undefined : 'subtle' }, text(lang, 'openTool')),
            )
          }),
        )
      }
      ...
      return h(
        ...
        h(
          ...
          h(Text, { bold: true, color: 'permission' }, `← ${text(lang, 'backToTools')}`),
        ),
        h(Box, { height: 1 }),
        h(Text, { bold: true, color: 'suggestion' }, `${clamp(toolIndex, server.tools.length) + 1}/${server.tools.length}  ${tool.name}`),
        h(Box, { marginTop: 1, flexDirection: 'column' },
          h(Text, { bold: true, color: 'permission' }, text(lang, 'description')),
          ...descriptionLines.map((line, index) => h(Text, { key: `description:${index}`, color: 'subtle', wrap: 'wrap' }, line || ' ')),
        ),
        h(Box, { marginTop: 1, flexDirection: 'column' },
          h(Text, { bold: true, color: 'permission' }, text(lang, 'schema')),
          ...schemaLines.map((line, index) => h(Text, { key: `schema:${index}`, color: 'subtle', wrap: 'wrap' }, line || ' ')),
        ),
      )
    }

    const renderDoctor = () => {
      if (busy === text(lang, 'doctorRunning')) return h(Text, { color: 'warning' }, text(lang, 'doctorRunning'))
      if (doctor === undefined) return h(Text, { color: 'subtle' }, text(lang, 'runDoctor'))
      return h(
        React.Fragment,
        null,
        ...doctor.checks.map((check: McpDoctorCheck) => h(
          Box,
          { key: check.id, flexDirection: 'column', marginBottom: 1 },
          h(
            Box,
            { flexDirection: 'row' },
          ...
          check.suggestion !== undefined && h(
            ...
            h(Text, { color: 'warning', wrap: 'wrap' }, `${text(lang, 'suggestion')}: ${doctorSuggestion(lang, check.suggestion)}`),
          ),
        )),
      )
    }

    const entries = (value: Record<string, unknown> | undefined): string => Object.keys(value ?? {}).join(', ') || '-'
    const renderConfig = (server: McpServerView) => h(
      React.Fragment,
      null,
      server.transport === 'stdio'
        ? h(React.Fragment, null,
            row(text(lang, 'command'), server.command ?? '-'),
            row(text(lang, 'arguments'), (server.args ?? []).join(' ') || '-'),
            row(text(lang, 'workingDirectory'), server.cwd ?? '-'),
            row(text(lang, 'environment'), entries(server.env)),
            row(text(lang, 'credentialRefs'), Object.values(server.secretEnv ?? {}).map((entry) => entry.ref).join(', ') || '-'),
          )
        : h(React.Fragment, null,
            row(text(lang, 'endpoint'), server.url ?? '-'),
            row(text(lang, 'headers'), entries(server.headers)),
    ...
    const renderSetEditor = (editor: SetEditorState) => h(
      React.Fragment,
      null,
      ...
      ...setEditorRows.map((item, index) => {
        ...
        if (item.kind === 'field') {
          ...
          return h(
            ...
            common,
            h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '),
            h(Text, { color: 'permission' }, `${text(lang, item.field === 'id' ? 'setId' : 'setName')}: `),
        ...
        if (item.kind === 'member') {
          ...
          return h(
            ...
            h(Box, { flexGrow: 1 }),
            h(Text, { color: stateColor(item.server.state) }, runtimeStateText(lang, item.server.state)),
    ...
    const renderSet = (set: McpSetView) => {
      ...
      return h(
        ...
        h(Box, { marginTop: 1, flexDirection: 'column' },
          ...members.map((server, index) => h(
            Box,
            { key: set.serverIds[index], flexDirection: 'row' },
            h(Text, { color: server === undefined ? 'error' : stateColor(server.state) }, server === undefined ? '? ' : `${stateGlyph(server.state)} `),
            h(Text, null, server?.name ?? set.serverIds[index]),
            h(Box, { flexGrow: 1 }),
            h(Text, { color: 'subtle' }, server === undefined ? '' : String(server.tools.length)),
          )),
        ),
        h(Box, { marginTop: 1 }, h(Text, { color: 'subtle' }, text(lang, 'setHint'))),
        h(Box, { height: 1 }),
        renderDetailAction(0, set.active ? '○' : '◆', text(lang, set.active ? 'disableSet' : 'enableSet'), set.active ? 'warning' : 'success'),
                ...
                : tab === 'tools'
                  ...
                  : tab === 'overview'
                    ? h(
                        React.Fragment,
                        null,
                        renderOverview(selected.server),
                        h(Box, { height: 1 }),
                        renderServerActions(selected.server),
                      )
      ...
      confirm === undefined && h(
        ...
        serverEditor !== undefined
          ...
          : h(React.Fragment, null,
              ...
              workspace === 'sets' && h(Text, { color: 'subtle' }, `a ${text(lang, 'createSet')}`),
              workspace === 'servers' && h(Text, { color: 'subtle' }, `a ${text(lang, 'createServer')}`),
              h(Text, { color: 'subtle' }, `r ${text(lang, 'refresh')}`),
