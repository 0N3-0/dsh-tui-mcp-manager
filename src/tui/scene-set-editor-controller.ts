...
export function setEditorRowsFor(
  editor: SetEditorState | undefined,
  servers: readonly McpServerView[],
): SetEditorRow[] {
  if (editor === undefined) return []
  const matchingServers = servers.filter((server) => matchesSearch(
    editor.memberFilter,
    server.name,
    server.id,
    server.serverName,
  ...
  return [
    ...
    { kind: 'search' },
    ...matchingServers.map((server: McpServerView) => ({ kind: 'member' as const, server })),
    { kind: 'save' },
    { kind: 'cancel' },
  ]
}

export function useSetEditorController({
  React,
  manager,
  lang,
  snapshot,
  ...
  scrollDetailTo,
  isMounted,
}: SetEditorControllerOptions) {
  const [editor, setEditor] = React.useState<SetEditorState | undefined>()
  ...
  const moveSelection = (delta: number): void => {
    setEditor((current: SetEditorState | undefined) => {
  ...
  const save = (current: SetEditorState): void => {
    ...
    if (current.mode === 'create' && snapshot?.sets.some((set: McpSetView) => set.id === record.id)) {
      const selected = rows.findIndex((row) => row.kind === 'field' && row.field === 'id')
      scrollDetailTo(selected)
    ...
    void mutate(
      () => manager.invoke('upsertSet', { set: record, active: current.draft.active }),
      text(lang, 'save'),
      text(lang, 'updated'),
    ).then((saved) => {
