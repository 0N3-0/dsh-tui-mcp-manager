...
export function setEditorRowsFor(
  ...
  return [
    ...
    ...servers.map((server: McpServerView) => ({ kind: 'member' as const, server })),
    { kind: 'save' },
    { kind: 'cancel' },
  ]
}
...
export function useSetEditorController({
  ...
  const save = (current: SetEditorState, index: number): void => {
  ...
  const activateRow = (index: number): void => {
    ...
    if (row.kind === 'field') {
      if (row.editable) setEditor({ ...editor, selected: index, editing: row.field, error: undefined })
      return
    }
    if (row.kind === 'member') {
