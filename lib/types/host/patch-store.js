...
function adapterConfig(record) {
    ...
    return {
        ...config,
        secretHeaders: Object.fromEntries(Object.entries(record.secretHeaders ?? {}).map(([key, value]) => [key, { ref: value.ref, prefix: value.prefix ?? '' }])),
    };
}
export function toLoaderEntry(input) {
...
function blockRange(content) {
    ...
    if (bodyEnd < bodyStart)
        throw new Error('dsh-mcp-manager managed block end appears before its start');
    const markerEnd = bodyEnd + ends[0][0].length;
    const end = markerEnd + (content[markerEnd] === '\n' ? 1 : 0);
    return { start, end, bodyStart, bodyEnd };
}
function parsePatchDocument(content, label) {
    let parsed;
    try {
    ...
    if (!Array.isArray(parsed))
        throw new Error(`${label} must contain a top-level YAML array`);
    return parsed;
}
function parseManagedServers(content, filename) {
    ...
    if (range === undefined)
        return { servers: [], hasManagedBlock: false, needsAdapterMigration: false };
    const value = parsePatchDocument(content.slice(range.bodyStart, range.bodyEnd), `${filename} managed block`);
    if (value.length !== 1)
        throw new Error('dsh-mcp-manager managed block must contain exactly one insert patch');
    ...
    for (const server of servers) {
        if (ids.has(server.id))
            throw new Error(`managed block contains duplicate server id ${JSON.stringify(server.id)}`);
...
function renderBlock(servers) {
    const patch = [{ insert: servers.map(toLoaderEntry) }];
