...
import { applyActiveSetsToServers, normalizeSetRecord, ProfileSetStore, removeServerFromSets, } from './set-store.js';
...
export class McpManagerService extends Service {
    ...
    revision = 0;
    changeNotificationQueued = false;
    chain = Promise.resolve();
    lastStorage;
    temporarilyStoppedIds = new Set();
    constructor(ctx) {
    ...
    installPatchFailureTracking() {
        ...
        this.ctx.on('hmr/config-update-failed', (filename, error) => {
            ...
            for (const record of this.records.values()) {
                ...
                if (this.entryMatches(record.config, entry))
                    continue;
                if (this.temporarilyStoppedIds.has(record.id) && this.entryMatches(record.config, entry, true))
                    continue;
                record.state = 'failed';
    ...
    async readSets(serverIds, activateInitialDefault = false) {
        if (this.setStore === undefined)
            return { sets: [], activeSetIds: [], initialized: true, writable: false, path: '' };
        ...
        if (!snapshot.initialized && serverIds !== undefined) {
            ...
            for (let index = 1; sets.some((set) => set.id === defaultId); index += 1)
                defaultId = `default-${index}`;
    ...
    async writeSets(sets, activeSetIds = []) {
        if (this.setStore === undefined)
            throw new ManagerError('active profile path is unavailable', { code: 'profile-unavailable' });
    ...
    entryMatches(record, entry, disabledOverride) {
        if (entry === undefined)
            return false;
    ...
    toolsFor(serverName, schemas = this.ctx.tools.schemas()) {
    ...
    refreshTools(record, schemas = this.ctx.tools.schemas()) {
    ...
    async startLoaderEntry(record, entry, config = toLoaderEntry(record.config).config) {
    ...
    async stopLoaderEntry(record, entry) {
     ...
     */
    async diagnoseDisabledRuntime(record) {
        const entry = this.loaderEntry(record.id);
    ...
    async doctor(id) {
        ...
        checks.push({
            ...
            ...(loaderApplied ? {} : { suggestion: 'reload-profile' }),
        });
        if (view.transport === 'stdio') {
            ...
            checks.push({
                ...
                ...(executable ? {} : { suggestion: 'edit-command' }),
            });
            if (view.cwd) {
                let cwdState = 'pass';
                let detail = `directory exists: ${view.cwd}`;
                try {
                    if (!(await stat(view.cwd)).isDirectory())
                ...
                checks.push({ id: 'cwd', state: cwdState, detail, ...(cwdState === 'fail' ? { suggestion: 'edit-cwd' } : {}) });
            }
        }
        else {
            let state = 'pass';
            ...
            checks.push({ id: 'target', state, detail, ...(state === 'fail' ? { suggestion: 'edit-url' } : {}) });
        }
        const credentialEntries = [
            ...Object.entries(view.secretEnv ?? {}).map(([name, item]) => [name, item.credential]),
            ...Object.entries(view.secretHeaders ?? {}).map(([name, item]) => [name, item.credential]),
        ];
        ...
        if (credentialEntries.length > 0) {
            checks.push({
                ...
                ...(missingCredentials.length > 0 ? { suggestion: 'set-credentials' } : {}),
            });
        }
        if (!view.enabled) {
            const preflightFailed = checks.some((check) => (check.state === 'fail'
            ...
            if (preflightFailed) {
                ...
                try {
                    ...
                    checks.push({
                        ...
                        ...(toolCount > 0 ? {} : { suggestion: 'reconnect-runtime' }),
                    });
                }
                catch (error) {
            ...
            const runtimeState = view.state === 'connected'
                ? 'pass'
                : view.state === 'failed'
                    ? 'fail'
                    : 'warn';
            ...
            checks.push({
                ...
                ...(temporarilyStopped || view.tools.length > 0
                    ? {}
                    : { suggestion: view.state === 'connected' ? 'reconnect-runtime' : 'wait-runtime' }),
            });
        }
        return {
            serverId: id,
        ...
        switch (endpoint) {
            ...
            case 'upsertSet': {
                ...
                await this.enqueue(async () => {
                    ...
                    if (unknown.length > 0)
                        throw new ManagerError(`set references unknown server(s): ${unknown.join(', ')}`, { code: 'not-found' });
                    const sets = await this.readSets(known);
                    const duplicateName = sets.sets.find((candidate) => candidate.id !== set.id && candidate.name.toLocaleLowerCase() === set.name.toLocaleLowerCase());
                    ...
                    if (requestedActive === true)
                        active.add(set.id);
                    else if (requestedActive === false)
                        active.delete(set.id);
            ...
            case 'removeSet': {
                ...
                await this.enqueue(async () => {
                    ...
                    if (!sets.sets.some((set) => set.id === id))
                        throw new ManagerError(`set ${JSON.stringify(id)} does not exist`, { code: 'not-found' });
            ...
            case 'remove': {
                ...
                await this.enqueue(async () => {
                    ...
                    const remainingServers = applyActiveSetsToServers(storage.servers.filter((server) => server.id !== id), remainingSets, sets.activeSetIds);
    ...
    async stopServer(id) {
        ...
        if (record === undefined)
            throw new ManagerError(`server ${JSON.stringify(id)} does not exist`, { code: 'not-found' });
        if (!record.config.enabled)
            throw new ManagerError('server is disabled by its Sets.', { code: 'disabled' });
        const entry = this.loaderEntry(id);
        if (this.temporarilyStoppedIds.has(id) && this.entryMatches(record.config, entry, true))
            return;
        if (entry === undefined || !this.entryMatches(record.config, entry)) {
    ...
    async viewFor(record, toolSchemas = this.ctx.tools.schemas()) {
        ...
        const [credentialEntries, secrets] = await Promise.all([
            Promise.all([...credentialRefs].map(async (ref) => [ref, await this.credentialState(ref)])),
    ...
    async snapshot() {
        ...
        const setStorage = await this.readSets(serverIds, (storage?.servers.length ?? 0) === 0 || storage.servers.every((server) => server.enabled));
...
async function resolveExecutable(command, cwd, configuredPath) {
    if (!command)
        return undefined;
    ...
    if (command.includes('/') || command.includes('\\')) {
        ...
        for (const extension of extensions)
            candidates.push(target.endsWith(extension) ? target : target + extension);
    }
    else {
...
function runtimeSuggestion(view) {
    const message = (view.error ?? '').toLowerCase();
    if (/\b(401|403|unauthori[sz]ed|forbidden|api[-_ ]?key|credential)\b/.test(message))
        return 'check-auth';
    if (/\b(enoent|command not found|spawn)\b/.test(message))
        return 'edit-command';
    if (/\b(timeout|timed out|econn|enotfound|dns|network|fetch failed|socket)\b/.test(message))
