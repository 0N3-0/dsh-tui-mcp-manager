...
export function renderServerEditorView({ React, ui, lang, snapshot, editor, rows, activateRow, }) {
    const { Box, Text } = ui;
    ...
    return h(React.Fragment, null, h(Text, { color: 'subtle', wrap: 'wrap' }, text(lang, 'serverEditorHint')), editor.error !== undefined && h(Box, { marginTop: 1, borderStyle: 'round', borderColor: 'error', paddingX: 1 }, h(Text, { color: 'error', wrap: 'wrap' }, editor.error)), h(Box, { height: 1 }), ...rows.map((item, index) => {
        ...
        if (item.kind === 'field') {
            ...
            return h(Box, common, marker, h(Text, { color: 'permission' }, `${serverFieldLabel(lang, item.field)}: `), h(Text, { bold: focused, color: item.editable ? undefined : 'subtle', wrap: 'truncate-end' }, `${value || text(lang, 'valueEmpty')}${editing ? '▍' : ''}`), h(Box, { flexGrow: 1 }), item.editable && h(Text, { color: 'subtle' }, text(lang, 'editValue')));
        }
        if (item.kind === 'transport') {
            return h(Box, common, marker, h(Text, { color: 'permission' }, `${text(lang, 'transport')}: `), h(Text, { bold: focused }, editor.draft.transport), h(Box, { flexGrow: 1 }), h(Text, { color: 'subtle' }, text(lang, 'toggle')));
        }
        if (item.kind === 'boolean') {
            const enabled = editor.draft[item.field];
            return h(Box, common, marker, h(Text, { color: 'permission' }, `${text(lang, item.field === 'failOnStartupError' ? 'failStartup' : 'reconnectEnabled')}: `), h(Text, { color: enabled ? 'success' : 'inactive', bold: focused }, text(lang, enabled ? 'enabled' : 'disabled')));
        }
        if (item.kind === 'credential') {
            ...
            return h(Box, common, marker, h(Text, { color: 'permission' }, `${text(lang, 'credentialValue')} ${item.ref}: `), h(Text, { color: pending !== undefined ? 'warning' : 'subtle', bold: focused }, editing ? `${'•'.repeat(Math.min(12, pending?.length ?? 0))}▍` : status));
        }
