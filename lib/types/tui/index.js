...
async function runManager(ctx, dialogs, manager, credentials) {
    const waitedForInitialSync = new Set();
    while (true) {
 ...
 */
async function settleInitialToolSync(manager, initial, waited) {
    ...
    do {
        await new Promise((resolve) => setTimeout(resolve, INITIAL_TOOL_SYNC_POLL_MS));
        snapshot = await manager.invoke('list', {});
        const stillStarting = ids.some((id) => snapshot.servers.some((server) => server.id === id && server.enabled && server.state === 'starting'));
        if (!stillStarting)
            break;
    } while (Date.now() < deadline);
    return snapshot;
}
async function runServerActions(ctx, dialogs, manager, credentials, serverId) {
    while (true) {
        const lang = await resolveTuiLanguage(ctx);
        let snapshot;
        try {
...
async function askForServer(dialogs, lang, snapshot, credentials, existing) {
...
async function showError(dialogs, lang, error) {
...
function stateLabel(lang, state) {
    const key = {
...
function serverDescription(lang, server) {
    return [
        server.serverName,
