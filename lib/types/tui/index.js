...
async function askForServer(dialogs, lang, snapshot, credentials, icons, existing) {
    ...
    const buildSteps = () => {
        const steps = [];
        if (!existing) {
            steps.push(inputStep('id', copy(lang, 'serverId'), copy(lang, 'serverIdPlaceholder'), () => draft.id, (value) => { draft.id = value; }, (value) => /^[a-z0-9][a-z0-9._-]{0,63}$/.test(value.trim()) || fail('invalidId')));
        }
