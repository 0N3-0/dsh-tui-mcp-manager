...
function runtimeStateText(lang, state) {
    const runtime = {
...
async function persistCredentialValues(credentials, values) {
...
function nextServerId(snapshot) {
...
function nextDuplicateId(snapshot, source) {
    const existing = new Set(snapshot.servers.map((server) => server.id));
    for (let index = 1;; index += 1) {
...
function nextDuplicateServerName(snapshot, source) {
 ...
 */
function parseSecretHeaderSpec(spec) {
