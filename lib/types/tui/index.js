...
import { persistCredentialValues } from './credential-provider.js';
...
import { formatArgs, formatEquals, formatSecretHeaders, hasPlainSecretEnv, hasPlainSecretHeader, isCredentialRefMap, isPairMap, isSecretHeaderMap, nextDuplicateId, nextDuplicateServerName, nextServerId, parseArgs, parseAssignments, parseCredentialRefs, parsePairs, parseSecretHeaders, } from './server-form-model.js';
...
export function applyTui(ctx) {
    ...
    ctx.inject(['tuiDialogs'], (tuiCtx) => {
        const dialogs = tuiCtx.get?.('tuiDialogs', false);
        const manager = tuiCtx.get?.('mcpManager', false);
            ...
            const definition = {
                ...
                handler: async () => {
                    if (scenes?.open(SCENE_ID))
                        return { kind: 'success' };
                    if (!credentials)
                        throw new Error('dsh-tui-mcp-manager: credentials service is required by the dialog fallback');
...
async function runManager(ctx, dialogs, manager, credentials, icons) {
    ...
    while (true) {
        ...
        if (choice.startsWith('set:')) {
            ...
            if (await runSetActions(ctx, dialogs, manager, credentials, icons, setId, collapsedSetIds.has(setId))) {
                if (collapsedSetIds.has(setId))
                    collapsedSetIds.delete(setId);
                else
                    collapsedSetIds.add(setId);
            }
            continue;
        }
        ...
        if (choice.startsWith('set-server:')) {
            ...
            if (setId && serverId)
                await runServerActions(ctx, dialogs, manager, credentials, icons, setId, serverId);
        }
    }
}
async function runServerPoolForSet(ctx, dialogs, manager, credentials, icons, setId) {
    const lang = await resolveTuiLanguage(ctx);
    const snapshot = await manager.invoke('list', {});
    const set = snapshot.sets.find((candidate) => candidate.id === setId);
    if (!set)
        return;
    const result = await runDraftServerPool(dialogs, lang, manager, credentials, icons, snapshot, setDisplayName(lang, set), set.serverIds);
...
async function runDraftServerPool(dialogs, lang, manager, credentials, icons, initialSnapshot, setName, initialServerIds) {
    ...
    while (true) {
        ...
        const widths = headers.map((header, index) => Math.max(terminalCellWidth(header), ...(columns[index] ?? []).map(terminalCellWidth)));
        const choice = await dialogs.select({
        ...
        if (!choice || choice === 'back')
            return { snapshot, serverIds: [...serverIds] };
        try {
            if (choice.startsWith('pool-server:')) {
                ...
                const action = await dialogs.select({
                    ...
                    options: [
                        {
                            id: 'membership',
                            label: iconLabel(serverIds.has(serverId) ? icons.remove : icons.add, copy(lang, serverIds.has(serverId) ? 'removeExistingFromSet' : 'addExistingToSet')),
                        },
...
async function runSetActions(ctx, dialogs, manager, credentials, icons, setId, collapsed) {
    while (true) {
        const lang = await resolveTuiLanguage(ctx);
        const snapshot = await manager.invoke('list', {});
        const set = snapshot.sets.find((candidate) => candidate.id === setId);
        ...
        try {
            ...
            if (choice === 'edit') {
                ...
                if (updated)
                    await manager.invoke('upsertSet', { set: updated });
                continue;
            }
            if (choice === 'delete') {
                const confirmed = await dialogs.confirm({
...
async function askForSet(dialogs, lang, snapshot, manager, credentials, icons, existing) {
    ...
    while (true) {
        ...
        const memberWidths = memberHeaders.map((header, index) => Math.max(terminalCellWidth(header), ...(memberColumns[index] ?? []).map(terminalCellWidth)));
        const choice = await dialogs.select({
        ...
        if (!choice || choice === 'cancel')
            return undefined;
        if (choice === 'members' || choice.startsWith('members-'))
            continue;
        ...
        if (choice === 'add-member') {
            const result = await runDraftServerPool(dialogs, lang, manager, credentials, icons, currentSnapshot, draft.name || draft.id, draft.serverIds);
...
async function runServerActions(ctx, dialogs, manager, credentials, icons, setId, serverId) {
    while (true) {
        const lang = await resolveTuiLanguage(ctx);
        let snapshot;
        try {
...
async function runDoctor(ctx, dialogs, manager, icons, serverId) {
    while (true) {
        const lang = await resolveTuiLanguage(ctx);
        let server;
        let report;
...
function doctorCheckValue(lang, check) {
    ...
    if (check.id === 'tools')
        return copy(lang, 'tools', { count: check.detail.split(' ')[0] ?? '0' });
    if (check.id === 'runtime' && !check.detail.includes(':')) {
        return runtimeStateText(lang, check.detail);
    }
    return check.detail;
}
function doctorSuggestion(lang, suggestion) {
    return copy(lang, doctorSuggestionStringKey(suggestion));
}
function doctorStateIcon(state, icons) {
    if (state === 'pass')
        return icons.enable;
    if (state === 'warn')
        return icons.reconnect;
    return icons.remove;
}
function doctorCheckLabel(lang, id) {
    return copy(lang, doctorCheckStringKey(id));
}
async function runInspector(ctx, dialogs, manager, icons, serverId) {
    while (true) {
        const lang = await resolveTuiLanguage(ctx);
        let server;
        try {
        ...
        if (!choice || choice === 'back')
            return;
        if (choice === 'overview')
            await showServerOverview(dialogs, lang, icons, server);
        if (choice === 'tools')
            await runToolInspector(dialogs, lang, icons, server);
        if (choice === 'error') {
            await showDetail(dialogs, copy(lang, 'latestError'), server.error ?? copy(lang, 'noError'), lang, icons);
        }
    }
}
async function showServerOverview(dialogs, lang, icons, server) {
    const credentials = [
...
async function runToolInspector(dialogs, lang, icons, server) {
    ...
    while (true) {
        ...
        if (tool)
            await showTool(dialogs, lang, icons, tool, server.serverName);
    }
}
async function showTool(dialogs, lang, icons, tool, serverName) {
    ...
    while (true) {
        ...
        if (choice === 'schema')
            await showDetail(dialogs, copy(lang, 'schema'), compactJson(schema), lang, icons);
        if (choice.startsWith('property:')) {
            const entry = entries[Number(choice.slice('property:'.length))];
            if (entry)
                await showDetail(dialogs, entry[0], schemaSummary(entry[1]), lang, icons);
        }
    }
}
async function runReadOnlyRows(dialogs, title, rows, lang, icons) {
    ...
    if (choice?.startsWith('row:')) {
        ...
        if (row)
            await showDetail(dialogs, row[0], row[1], lang, icons);
    }
}
async function showDetail(dialogs, title, value, lang, icons) {
...
function shortToolName(name, serverName) {
...
function schemaType(value) {
    ...
    if (Array.isArray(value.type))
        return value.type.map(String).join('|');
...
function schemaSummary(value) {
    ...
    if (value.default !== undefined)
        parts.push(`default=${compactJson(value.default)}`);
    if (Array.isArray(value.enum))
        parts.push(`enum=${value.enum.map(String).join(', ')}`);
    return parts.join(' | ');
}
...
async function askForServer(dialogs, lang, snapshot, credentials, icons, existing, intent = existing ? 'edit' : 'add') {
    ...
    const inputStep = (key, title, placeholder, get, set, validate) => ({
        key,
        title,
    ...
    const booleanStep = (key, title, get, set) => ({
        key,
        title,
        async run(decoratedTitle) {
            const current = get();
    ...
    const buildSteps = () => {
        const steps = [];
        if (intent !== 'edit') {
            steps.push(inputStep('id', copy(lang, 'serverId', { mode }), copy(lang, 'serverIdPlaceholder'), () => draft.id, (value) => { draft.id = value; }, (value) => /^[a-z0-9][a-z0-9._-]{0,63}$/.test(value.trim()) || fail('invalidId')));
        }
        steps.push(inputStep('displayName', copy(lang, 'displayName', { mode }), copy(lang, 'displayNamePlaceholder'), () => draft.displayName, (value) => { draft.displayName = value; }), inputStep('serverName', copy(lang, 'serverName', { mode }), copy(lang, 'serverNamePlaceholder'), () => draft.serverName, (value) => { draft.serverName = value; }, (value) => /^[A-Za-z0-9_-]{1,32}$/.test(value.trim()) || fail('invalidServerName')), {
            key: 'transport',
            title: copy(lang, 'transport', { mode }),
            async run(title) {
                const choices = ['stdio', 'streamable-http'];
                const value = await dialogs.select({
                    title,
        ...
        if (draft.transport === 'stdio') {
            steps.push(inputStep('command', copy(lang, 'command', { mode }), copy(lang, 'commandPlaceholder'), () => draft.command, (value) => { draft.command = value; }, (value) => value.trim() !== '' || fail('invalidCommand')), inputStep('args', copy(lang, 'args', { mode }), copy(lang, 'argsPlaceholder'), () => draft.args, (value) => { draft.args = value; }), inputStep('cwd', copy(lang, 'cwd', { mode }), copy(lang, 'cwdPlaceholder'), () => draft.cwd, (value) => { draft.cwd = value; }), inputStep('env', copy(lang, 'env', { mode }), copy(lang, 'envPlaceholder'), () => draft.env, (value) => { draft.env = value; }, async (value) => {
                if (!isPairMap(value))
                    return fail('invalidPairs');
                if (hasPlainSecretEnv(value))
                    return fail('invalidSecretEnv');
                return true;
            }), inputStep('secretEnv', copy(lang, 'secretEnv', { mode }), copy(lang, 'secretEnvPlaceholder'), () => draft.secretEnv, (value) => { draft.secretEnv = value; }, (value) => isCredentialRefMap(value) || fail('invalidCredentialRefs')));
        }
        else {
            steps.push(inputStep('url', copy(lang, 'url', { mode }), copy(lang, 'urlPlaceholder'), () => draft.url, (value) => { draft.url = value; }, (value) => value.trim() !== '' || fail('invalidUrl')), inputStep('headers', copy(lang, 'headers', { mode }), copy(lang, 'headersPlaceholder'), () => draft.headers, (value) => { draft.headers = value; }, async (value) => {
                if (!isPairMap(value))
                    return fail('invalidPairs');
                if (hasPlainSecretHeader(value))
                    return fail('invalidSecretHeaders');
                return true;
            }), inputStep('secretHeaders', copy(lang, 'secretHeaders', { mode }), copy(lang, 'secretHeadersPlaceholder'), () => draft.secretHeaders, (value) => { draft.secretHeaders = value; }, (value) => isSecretHeaderMap(value) || fail('invalidSecretHeaders')));
        }
        ...
        steps.push(inputStep('toolCallTimeoutMs', copy(lang, 'formToolTimeout'), '60000', () => String(draft.toolCallTimeoutMs), (value) => { draft.toolCallTimeoutMs = Number(value); }, (value) => positiveNumber(value) !== undefined || fail('invalidPositiveNumber')), booleanStep('failOnStartupError', copy(lang, 'formFailStartup'), () => draft.failOnStartupError, (value) => { draft.failOnStartupError = value; }), booleanStep('reconnectEnabled', copy(lang, 'formReconnect'), () => draft.reconnect.enabled, (value) => { draft.reconnect.enabled = value; }), inputStep('reconnectInitialDelayMs', copy(lang, 'formInitialDelay'), '500', () => String(draft.reconnect.initialDelayMs), (value) => { draft.reconnect.initialDelayMs = Number(value); }, (value) => positiveNumber(value) !== undefined || fail('invalidPositiveNumber')), inputStep('reconnectMaxDelayMs', copy(lang, 'formMaxDelay'), '30000', () => String(draft.reconnect.maxDelayMs), (value) => { draft.reconnect.maxDelayMs = Number(value); }, (value) => positiveNumber(value) !== undefined || fail('invalidPositiveNumber')), inputStep('reconnectMaxAttempts', copy(lang, 'formMaxAttempts'), '10', () => String(draft.reconnect.maxAttempts), (value) => { draft.reconnect.maxAttempts = Number(value); }, (value) => positiveInteger(value) !== undefined || fail('invalidPositiveInteger')));
        return steps;
    };
    const labelFor = (key) => {
        if (key.startsWith('credential:'))
            return copy(lang, 'formCredential', { ref: key.slice('credential:'.length) });
        const labels = {
            id: 'formId',
    ...
    const present = (value) => value.trim() === '' ? copy(lang, 'valueEmpty') : value;
    const count = (value, key) => copy(lang, key, {
        count: key === 'valueArgs' ? parseArgs(value).length : (parseAssignments(value) ?? []).length,
    });
    const summaryFor = async (key) => {
        if (key.startsWith('credential:')) {
            const ref = key.slice('credential:'.length);
            if (draft.credentialValues[ref] !== undefined)
                return copy(lang, 'valuePending');
            try {
    ...
    const validateForm = async () => {
        if (!/^[a-z0-9][a-z0-9._-]{0,63}$/.test(draft.id.trim()))
            return fail('invalidId');
        if (!/^[A-Za-z0-9_-]{1,32}$/.test(draft.serverName.trim()))
            return fail('invalidServerName');
        ...
        if (draft.transport === 'stdio') {
            ...
            if (!isCredentialRefMap(draft.secretEnv))
                return fail('invalidCredentialRefs');
        }
        else {
        ...
        if (positiveNumber(String(draft.toolCallTimeoutMs)) === undefined)
            return fail('invalidPositiveNumber');
        if (positiveNumber(String(draft.reconnect.initialDelayMs)) === undefined)
            return fail('invalidPositiveNumber');
        if (positiveNumber(String(draft.reconnect.maxDelayMs)) === undefined)
            return fail('invalidPositiveNumber');
        if (positiveInteger(String(draft.reconnect.maxAttempts)) === undefined)
            return fail('invalidPositiveInteger');
        if (draft.reconnect.initialDelayMs > draft.reconnect.maxDelayMs)
            return fail('invalidReconnectDelays');
        for (const ref of credentialReferences()) {
...
async function ask(dialogs, title, placeholder, initial = '') {
    return dialogs.input({ title, placeholder, initial, timeoutMs: DIALOG_TIMEOUT_MS });
}
async function showError(dialogs, lang, icons, error) {
    await dialogs.select({
...
function serverListOptions(lang, servers, icons, prefix = '', idPrefix = 'server:', alignmentServers = servers, hasFollowingSibling = false) {
    const names = servers.map((server) => truncateToCells(server.name, 24));
    const toolCounts = servers.map((server) => server.tools.length === 0 && (server.state === 'starting' || server.state === 'reconnecting')
        ? '...'
...
function stateTag(state, icons) {
    return icons.states[state];
}
function setDisplayName(lang, set) {
...
const PAD_CELL = '\u2800';
function dialogSpacer(text, targetWidth) {
...
function truncateToCells(text, maxWidth) {
    ...
    for (const character of text) {
        ...
        if (width + nextWidth > contentWidth)
            break;
        result += character;
        width += nextWidth;
    }
...
function characterCellWidth(character) {
    ...
    if (codePoint === 0
        ...
        return 0;
    if (/\p{Extended_Pictographic}/u.test(character) || isWideCodePoint(codePoint))
        return 2;
    return 1;
}
