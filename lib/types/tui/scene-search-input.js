...
const SEARCH_PREFIX = '\uf002 ';
function elementScreenPosition(node) {
    let x = 0;
    let y = 0;
    let current = node;
...
export function SceneSearchInput({ React, ui, query, cursor, maxWidth = 32, compact = false, beginSearch, }) {
    const { Box, Text } = ui;
    ...
    React.useLayoutEffect(() => {
        if (cursor === undefined || process.stdout.isTTY !== true)
            return;
        ...
        queueMicrotask(() => {
            if (inputRef.current !== element || process.stdout.destroyed)
                return;
    ...
    return h(Box, {
        ...
        onClick: beginSearch,
    }, h(Text, { wrap: 'truncate-end' }, SEARCH_PREFIX, cursor === undefined
        ? query
        : h(React.Fragment, null, segments?.before, h(Text, { inverse: true }, segments?.cursor), segments?.after)));
}
//# sourceMappingURL=scene-search-input.js.map