...
export function renderServerDetailView({ React, ui, lang, server, tab, focusArea, toolIndex, toolDetailOpen, detailScrollHeight, doctor, busy, setToolDetailOpen, setFocusArea, scrollDetailTo, openTool, }) {
    const { Box, Text } = ui;
    ...
    if (tab === 'tools') {
        ...
        if (!toolDetailOpen) {
            ...
            return h(React.Fragment, null, h(Text, { bold: true, color: 'permission' }, `${text(lang, 'toolList')}  ${server.tools.length}`), ...visible.items.map((item, offset) => {
