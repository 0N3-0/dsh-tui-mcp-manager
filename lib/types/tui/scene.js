...
export function createMcpManagerScene(manager, resolveLanguage, credentials) {
    return function McpManagerScene(props) {
        ...
        const renderSetActions = () => selectedSet === undefined ? null : h(React.Fragment, null, renderDetailAction(0, selectedSet.active ? '○' : '◆', text(lang, selectedSet.active ? 'disableSet' : 'enableSet'), selectedSet.active ? 'warning' : 'success'), renderDetailAction(1, '\u2192', text(lang, 'editSet'), 'suggestion'), renderDetailAction(2, '×', text(lang, 'deleteSet'), 'error'));
