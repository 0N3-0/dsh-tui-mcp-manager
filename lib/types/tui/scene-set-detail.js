...
export function setEditorSelectionHelp(lang, editor, rows) {
    const row = rows[editor.selected];
    if (row?.kind === 'field')
        return text(lang, row.field === 'id' ? 'helpSetId' : 'helpSetName');
    if (row?.kind === 'boolean')
        return text(lang, 'helpEnableAtStartup');
    if (row?.kind === 'search')
        return text(lang, 'helpSearchMembers');
    if (row?.kind === 'member')
        return text(lang, 'helpSetMember').replace('{server}', row.server.name);
    return text(lang, row?.kind === 'save' ? 'helpSave' : 'helpCancel');
}
export function renderSetEditorView({ React, ui, lang, snapshot, editor, rows, rowWidth, activateRow, }) {
    const { Box, Text } = ui;
    ...
    return h(React.Fragment, null, h(Text, { color: 'subtle', wrap: 'wrap' }, text(lang, 'setEditorHint')), h(Text, { color: 'permission' }, `${text(lang, 'setMembers')}: ${editor.draft.serverIds.length}/${snapshot?.servers.length ?? 0}`), editor.error !== undefined && h(Box, { marginTop: 1, borderStyle: 'round', borderColor: 'error', paddingX: 1 }, h(Text, { color: 'error', wrap: 'wrap' }, editor.error)), h(Box, { height: 1 }), ...rows.map((item, index) => {
        ...
        if (item.kind === 'field') {
            ...
            return h(Box, common, h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '), h(Text, { color: value.trim() === '' ? 'error' : 'warning' }, '* '), h(Text, { color: 'permission' }, label), h(Text, {
                bold: focused,
        ...
        if (item.kind === 'member') {
            ...
            return h(Box, common, h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '), h(Text, null, '  '), h(Text, { color: member ? 'success' : 'inactive' }, `${member ? '\u2713' : '\u25cb'} `), h(Text, { bold: focused, wrap: 'truncate-end' }, item.server.name), h(Box, { flexGrow: 1 }), h(Text, { color: serverStateColor(item.server.state) }, runtimeStateText(lang, item.server.state)), h(Text, { color: 'subtle' }, `  ${item.server.tools.length}`));
        }
        if (item.kind === 'search') {
            ...
            const cursor = searching
                ...
                : undefined;
            return h(Box, common, h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '), h(Text, null, '  '), h(Text, { color: 'permission' }, label), h(Text, { bold: focused, color: value === '' ? 'subtle' : undefined, wrap: 'truncate-end' }, searching
                ? h(React.Fragment, null, cursor?.before, h(Text, { inverse: true }, cursor?.cursor), cursor?.after)
                : truncateTerminalText(value || text(lang, 'allItems'), valueWidth)), h(Box, { flexGrow: 1 }), h(Text, { color: matchCount === 0 ? 'warning' : 'subtle' }, count));
        }
        if (item.kind === 'boolean') {
            const enabled = editor.draft[item.field];
            return h(Box, common, h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '), h(Text, null, '  '), h(Text, { color: 'permission' }, `${text(lang, 'enableAtStartup')}: `), h(Text, { color: enabled ? 'success' : 'inactive', bold: focused }, text(lang, enabled ? 'enabled' : 'disabled')), h(Box, { flexGrow: 1 }), h(Text, { color: 'subtle' }, text(lang, 'toggleOption')));
        }
...
export function renderSetDetailView({ React, ui, lang, snapshot, set, focusArea, selectedMemberIndex, activateMember, }) {
    const { Box, Text } = ui;
    const h = React.createElement;
    ...
    return h(React.Fragment, null, row(text(lang, 'active'), text(lang, set.active ? 'active' : 'inactive'), set.active ? 'success' : 'inactive'), row(text(lang, 'members'), String(set.serverIds.length)), h(Box, { marginTop: 1, flexDirection: 'column' }, ...members.map((server, index) => {
        ...
        }, h(Text, { color: focused ? undefined : 'subtle' }, focused ? '\u276f ' : '  '), h(Text, { color: focused ? undefined : server === undefined ? 'error' : serverStateColor(server.state) }, server === undefined ? '? ' : `${serverStateGlyph(server.state)} `), h(Text, {
            bold: focused || server?.enabled === true,
            color: focused ? undefined : server?.enabled ? 'suggestion' : undefined,
        }, server?.name ?? set.serverIds[index]), h(Box, { flexGrow: 1 }), h(Text, { color: focused ? undefined : 'subtle' }, server === undefined ? '' : String(server.tools.length)));
